...
class Formo_Element_Core {
	...
	{
		Event::run('formoel.preload', $this);
	...
	/**
	 * Magic __get method. This avoids pointless errors when working with form templates.
	 *
	 * @return NULL
	 */		
	...
	{
		return '<input type="text" name="'.$this->name.'" value="'.$this->value.'"'.Formo::quicktagss($this->_find_tags()).'" />';
	}
	...
	{	
		return '<input type="submit" name="'.preg_replace('/ /','_',$this->name).'" value="'.$this->value.'"'.Formo::quicktagss($this->_find_tags()).' />';
	}
	...
	public function validate()
	{
		...
		elseif ($this->type == 'checkbox')
			return $this->_validate_checkbox();
	...
	private function _validate_checkbox()
	{
