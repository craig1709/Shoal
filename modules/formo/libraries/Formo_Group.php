...
class Formo_Group_Core {
	 ...
	 *
	 */	
	public function __construct($type, $name, $values, $info=array())
	{
		...
		$this->_add_values($values, $info);	
	}
	
	private function _add_values($values, $info)
	{
	...
	public function find_checked($type)
	{
		$this->post_type = $type;
		switch ($this->el_type)
		{
	...
	public function get($inside_only = FALSE)
	{
		foreach ($this->elements as $element=>$type)
		{
			...
			$this->$element->$label_place = ' '.preg_replace($search, '', $this->$element->label).' ';
			$this->$element->$space_place = $this->element_space;
